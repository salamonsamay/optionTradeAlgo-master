...
// more dn work, reqHistoricalData_.g. deltaNeutralValidation
...
// When a child order is transmitted with a different contract than the parent but no hedge params it sort of works but not really, reqHistoricalData_.g. contract does not display at TWS, but order is working
// Switch between frozen and real-time quotes is broken; reqHistoricalData_.g.: request frozen quotes, then realtime, then request option market data; you don't get bid/ask; request frozen, then an option; you don't get anything
...
// TWS changes
// TWS sends acct updateProccess time after every value; not necessary
