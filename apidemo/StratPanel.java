...
public class StratPanel extends StackPanel implements IHistoricalDataHandler, IRealTimeBarHandler {
	...
	StratPanel() {
		...
		add( p1, "Go long", sp(5), m_shares, sp(5), "shares when ask goes above MACDRequest by", sp(5), m_pct1, "%");
		
		JPanel p2 = new HPanel();
		add( p2, "Go flat when bid goes below MACDRequest by", sp(5), m_pct2, "%");
		
		JPanel p3 = new HPanel();
		add( p3, "MACDRequest bar size:", sp(5), m_barSize, sp(20), "MACDRequest number of bars", sp(5), m_bars);
